...
- **역할**: 유튜브 영상 또는 블로그(네이버, 티스토리 등) 링크를 제공받아, 본문 속 언급된 장소를 추출하고 6대 카테고리로 자동 분류하여 저장함에 반영합니다.
- **6대 카테고리**:
  1. `식당 및 카페`
  2. `숙박`
  3. `유적지 및 관광지`
...
- **저장 위치**:
  ...
  - `data/mymaps/내지도_*.csv`: 구글 내 지도(My Maps) 레이어 업로드용 CSV 자동 동기화
- **연동 모듈 & 실행 명령어**:
  - `python3 cli.py add-link <URL> [--region <도시>]`
  - 에이전트 내부 함수: `src.place_extractor.process_url_and_save_places`
...
- **핵심 알고리즘**:
  ...
  - **지도 링크 포함**: 각 일정 항목마다 바로 열어볼 수 있는 구글 지도 검색 링크(`https://www.google.com/maps/search/?api=1&query=...`)를 제공합니다.
- **저장 위치**:
